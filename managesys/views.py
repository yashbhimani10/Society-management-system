...
def rec2(request,username,Month):
    data20=user_reg.objects.get(uname=username)
    data21=maintain.objects.all()
    data22=maintain.objects.get(Month=Month,Name=username)
    context={"data20":data20, "data21":data21, "data22":data22}
    for i in data21:
        if i.Name==username:
            return render(request, 'rec2.html', context)
        else:
            pass
    return homeuser(request,username)
